...
final class wp_easycart_admin_products{
	...
	public function save_new_quick_product( ){
		if( current_user_can( 'manage_options' ) ){
			global $wpdb;
			$status = $_POST['ec_new_product_status'];
			$activate_in_store = ( $status == '1' || $status == '2' ) ? 1 : 0;
			$post_status = ( $activate_in_store ) ? 'publish' : 'private';
			$show_on_startup = ( $status == '1' ) ? 1 : 0;
			...
			/* Insert Product */
			$wpdb->query( $wpdb->prepare( "INSERT INTO ec_product( activate_in_store, show_on_startup, title, model_number, manufacturer_id, price, image1, post_id, show_stock_quantity, use_advanced_optionset, option_id_1, option_id_2, option_id_3, option_id_4, option_id_5 ) VALUES( %d, %d, %s, %s, %d, %s, %s, %d, 0, %d, %d, %d, %d, %d, %d )", $activate_in_store, $show_on_startup, $title, $sku, $manufacturer, $price, $image, $post_id, $use_advanced_optionset, $option1, $option2, $option3, $option4, $option5 ) );
	...
	public function add_advanced_option( ){
		if( current_user_can( 'manage_options' ) ){
			...
			$wpdb->query( $wpdb->prepare( "INSERT INTO ec_option_to_product( product_id, option_id ) VALUES( %d, %d )", $product_id, $option_id ) );
			return $wpdb->insert_id;
		}
	}
	...
	public function run_importer() {
		...
		if( $_POST['import_file_url'] ) {
			//echo $_POST['import_file_url'];

			set_time_limit( 500 );
			
			//if( !file_exists( $_POST['import_file_url'] ) ){
				//echo "Import file not found on server or did not upload successfully.";
			//}
			
			$file = fopen(  $_POST['import_file_url'] , "r" );
