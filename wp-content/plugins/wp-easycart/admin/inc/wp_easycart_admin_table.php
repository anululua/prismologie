...
final class wp_easycart_admin_table{
	...
	public function __construct(){ 
		...
		if( isset( $_GET['perpage'] ) ){
			...
			$_SESSION['ec_admin_perpage'] = $this->perpage;
		}else if( isset( $_SESSION['ec_admin_perpage'] ) ){
			$this->perpage = $_SESSION['ec_admin_perpage'];
		}else
			$this->perpage = 25;
	...
	public function print_table( ){
		...
		if($this->importer) {
			echo '<a onclick="ec_admin_importer_open_close(\'' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_importer\');" class="ec_page_title_button">'.$this->importer_button.'</a>';
			//import div
			echo '<div id="' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_importer" class="ec_importer_form">';
			//textbox for file url
			echo '<a href="' . wp_easycart_admin( )->helpsystem->print_docs_url($this->docs_guide, htmlspecialchars( $this->docs_link, ENT_QUOTES ), "importer" ) . '" target="_blank" class="ec_admin_importer_help_link">Need Help?</a> <input type="text" name="' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_import_file" id="' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_import_file"  class="wpec-admin-upload-input" />';
			//browse button to open wp media manager
			echo '<input type="button" class="ec_page_title_button" value="Browse" id="' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_browse_button" onclick="ec_admin_import_file_upload( \'' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_import_file\', \'' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_import_button\', \'' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_importer_status\');" />';
			//import button
			echo '<input type="button" class="ec_page_title_button ec_import_button" value="Import File" id="' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_import_button" onclick="ec_admin_start_importer( \'' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_import_file\', \'' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_importer_status\');" />';
			echo '</div>';
			//status window
			echo '<div id="' . htmlspecialchars( $_GET['subpage'], ENT_QUOTES ) . '_importer_status" class="ec_importer_status">';
			echo '</div>';
