...
final class wp_easycart_admin{
	...
	public function change_uploads_dir( ){
		...
		add_filter( 'update_attached_file', array( $this, 'custom_download_attached_meta' ), 10, 2 );
	}
	
	public function custom_download_location( $upload ){
		if( isset( $_REQUEST['is_wpec_download'] ) && $_REQUEST['is_wpec_download'] == '1' ){
			$upload['basedir'] = WP_PLUGIN_DIR;
			$upload['baseurl'] = WP_PLUGIN_URL;
			$upload['subdir']  = "/wp-easycart-data/products/downloads";
			$upload['path']    = WP_PLUGIN_DIR . "/wp-easycart-data/products/downloads";
			$upload['url']     = WP_PLUGIN_URL . "/wp-easycart-data/products/downloads";
		}
		return $upload;
	}
	
	public function custom_download_attached_meta( $file, $id ){
		if( isset( $_REQUEST['is_wpec_download'] ) && $_REQUEST['is_wpec_download'] == '1' ){
	...
	private function get_month_sales_total( ){
		...
		if( $local_tz_diff < 0 ){
			return $this->wpdb->get_var( "SELECT SUM( ec_orderdetail.total_price ) as total FROM ec_orderdetail LEFT JOIN ec_order ON ec_order.order_id = ec_orderdetail.order_id LEFT JOIN ec_orderstatus ON ec_orderstatus.status_id = ec_order.orderstatus_id WHERE MONTH( DATE_SUB( UTC_TIMESTAMP( ), INTERVAL " . ($local_tz_diff*-1) . " HOUR ) ) = MONTH( ec_order.order_date ) AND YEAR( DATE_SUB( UTC_TIMESTAMP( ), INTERVAL " . ($local_tz_diff*-1) . " HOUR ) ) = YEAR( ec_order.order_date ) AND ec_orderstatus.`is_approved` = 1" );
		}else{
			return $this->wpdb->get_var( "SELECT SUM( ec_orderdetail.total_price ) as total FROM ec_orderdetail LEFT JOIN ec_order ON ec_order.order_id = ec_orderdetail.order_id LEFT JOIN ec_orderstatus ON ec_orderstatus.status_id = ec_order.orderstatus_id WHERE MONTH( DATE_ADD( UTC_TIMESTAMP( ), INTERVAL " . $local_tz_diff . " HOUR ) ) = MONTH( ec_order.order_date ) AND YEAR( DATE_ADD( UTC_TIMESTAMP( ), INTERVAL " . $local_tz_diff . " HOUR ) ) = YEAR( ec_order.order_date ) AND ec_orderstatus.`is_approved` = 1" );
		}
	}
	...
	private function get_month_percentage_change( ){
		$local_tz_diff = get_option( 'gmt_offset' );
		if( $local_tz_diff < 0 ){
			$last_month = $this->wpdb->get_var( "SELECT SUM( ec_orderdetail.total_price ) as total FROM ec_orderdetail LEFT JOIN ec_order ON ec_order.order_id = ec_orderdetail.order_id LEFT JOIN ec_orderstatus ON ec_orderstatus.status_id = ec_order.orderstatus_id WHERE MONTH( DATE_SUB( DATE_SUB( UTC_TIMESTAMP( ), INTERVAL " . ($local_tz_diff*-1) . " HOUR ), INTERVAL 30 DAY ) ) = MONTH( ec_order.order_date ) AND YEAR( DATE_SUB( DATE_SUB( UTC_TIMESTAMP( ), INTERVAL " . ($local_tz_diff*-1) . " HOUR ), INTERVAL 30 DAY ) ) = YEAR( ec_order.order_date ) AND ec_orderstatus.`is_approved` = 1" );
		}else{
			$last_month = $this->wpdb->get_var( "SELECT SUM( ec_orderdetail.total_price ) as total FROM ec_orderdetail LEFT JOIN ec_order ON ec_order.order_id = ec_orderdetail.order_id LEFT JOIN ec_orderstatus ON ec_orderstatus.status_id = ec_order.orderstatus_id WHERE MONTH( DATE_SUB( DATE_ADD( UTC_TIMESTAMP( ), INTERVAL " . $local_tz_diff . " HOUR ), INTERVAL 30 DAY ) ) = MONTH( ec_order.order_date ) AND YEAR( DATE_SUB( DATE_ADD( UTC_TIMESTAMP( ), INTERVAL " . $local_tz_diff . " HOUR ), INTERVAL 30 DAY ) ) = YEAR( ec_order.order_date ) AND ec_orderstatus.`is_approved` = 1" );
		}
	...
	public function load_settings_content( ){
		...
		if( isset( $_GET['subpage'] ) && $_GET['subpage'] == "setup-wizard" ){
			...
			$cart = new wp_easycart_admin_cart_importer( );
			$cart->load_cart_importer( );
	...
	public function load_status_content( ){
		$store_status = new wp_easycart_admin_store_status( );
		$store_status->load_status( );
	}
	...
	public function elementor_check( ){
		...
		if( file_exists( $elementor_plugin_file ) && is_plugin_active( $elementor_plugin_base ) ) {
			echo '<div class="updated">';
			echo '<p>Elementor can cause issues with WP EasyCart. If you are having issues, please <a href="http://docs.wpeasycart.com/wp-easycart-administrative-console-guide/?section=elementor-page-builder" target="_blank">read more by clicking here</a> as the issue is easy to resolve!';
			echo '</div>';
		}
	}
	...
	public function load_upsell_image( ){
		if( !isset( $_GET['subpage'] ) || ( isset( $_GET['subpage'] ) && $_GET['subpage'] != 'setup-wizard' ) ){
			$image_id = rand( 1, 2 );
			if( $image_id == 1 )
				echo '<div style="width:100%; text-align:center; max-width:100%;"><a href="https://www.wpeasycart.com/wp-easycart-professional-14-no-risk-trial/?upsell=1" target="_blank"><img src="' . plugins_url( 'wp-easycart/admin/images/banner-ad-1-750x100.jpg' ) . '" style="max-width:100%; height:auto;" alt="Start Your PRO Trial Today!" /></a></div>';
			else
				echo '<div style="width:100%; text-align:center; max-width:100%;"><a href="https://www.wpeasycart.com/wordpress-shopping-cart-pricing/?upsell=2" target="_blank"><img src="' . plugins_url( 'wp-easycart/admin/images/banner-ad-2-750x100.jpg' ) . '" style="max-width:100%; height:auto;" alt="Buy WP EasyCart PRO today!" /></a></div>';
		}
	}
	...
	public function show_upgrade( ){
		include( WP_PLUGIN_DIR . "/" . EC_PLUGIN_DIRECTORY . '/admin/template/upgrade/upgrade-screen.php' );
	}
