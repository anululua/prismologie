	...
	paypal.Button.render({
		...
		payment: function(data, actions) {
			...
			var paymentPromise = new paypal.Promise( function( resolve, reject ){
				...
				//$http.post( myServerURL, myData ).then( function( response ){
			...
			<?php /*return actions.order.create({
				order: {
					intent: '',
					purchase_units: [
						{
							reference_id: 'WPEASYCART_ORDER',
							...
							partner_fee_details: {
								receiver: {
									email: '<?php echo ( get_option( 'ec_option_paypal_use_sandbox' ) == '1' ) ? 'paypalpro@wpeasycart.com' : 'paypal-partner@wpeasycart.com'; ?>',
									payee_display_metadata: {
										brand_name: 'WP EasyCart',
										email: '<?php echo ( get_option( 'ec_option_paypal_use_sandbox' ) == '1' ) ? 'paypalpro@wpeasycart.com' : 'paypal-partner@wpeasycart.com'; ?>'
									},
									merchant_id: '<?php echo ( get_option( 'ec_option_paypal_use_sandbox' ) == '1' ) ? '3V2SXZAGVD53L' : 'U4HGH5W64EUBC'; ?>'
								},
								amount: {
									value: '<?php echo number_format( $this->order_totals->grand_total * $fee_rate / 100, 2, '.', '' ); ?>',
									currency: '<?php echo $paypal_currency; ?>'
								}
							}<?php }?>
						}
