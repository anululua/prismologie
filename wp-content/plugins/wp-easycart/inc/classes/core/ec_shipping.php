...
class ec_shipping{
	...
	private function get_fraktjakt_based_shipping_options( ){
		...
		foreach( $this->fraktjakt_shipping_options as $shipping_option ){
			...
			$ret_string .= ">" . $shipping_option['description'] . " (" . $GLOBALS['currency']->get_symbol( ) . "<span id=\"ec_cart_standard_shipping_price\">" . $GLOBALS['currency']->get_number_only( $shipping_option['price'] + $this->handling ) . "</span>)</div>";
			$i++;
		}
		return $ret_string;
	...
	private function get_live_based_radio( $count, $i, $rate, $service_days = 0 ){
		...
		if( $rate != "ERROR" ){
			...
			$ret_string .= "<div class=\"ec_cart_shipping_method_row\">";
			$ret_string .= "<input type=\"radio\" class=\"no_wrap\" name=\"ec_cart_shipping_method\" value=\"" . $this->live_based[$i][2] . "\" onchange=\"ec_cart_shipping_method_change('" . $this->live_based[$i][2] . "', " . $rate . " ); \"";
	...
	public function get_single_shipping_price_content( $standard_text, $express_text, $standard_price ){
		...
		if( $GLOBALS['ec_cart_data']->cart_data->shipping_method == "" || $GLOBALS['ec_cart_data']->cart_data->shipping_method == "standard" )
			$ret_string .= " checked=\"checked\"";
		$ret_string .= " /><span class=\"ec_cart_standard_shipping_price_label\">" . $standard_text . " (" . $GLOBALS['currency']->get_symbol( ) . "</span><span id=\"ec_cart_standard_shipping_price\">" . $GLOBALS['currency']->get_number_only( $standard_price + $this->handling - $shipping_discount ) . "</span>)</div>";
		if( get_option( 'ec_option_add_local_pickup' ) ){
			...
			if( $GLOBALS['ec_cart_data']->cart_data->shipping_method == "free" )
				$ret_string .= " checked=\"checked\"";
			$ret_string .= " /><span class=\"ec_cart_standard_shipping_price_label\">" . $GLOBALS['language']->get_text( 'cart_estimate_shipping', 'cart_estimate_shipping_free' ) . " (" . $GLOBALS['currency']->get_symbol( ) . "</span> <span id=\"ec_cart_standard_shipping_price_free\">" . $GLOBALS['currency']->get_number_only( 0 ) . "</span>)</div>";
		}
		if( $this->express_price > 0 ){
			...
			if( $this->ship_express )
				$ret_string .= " checked=\"checked\"";
			$ret_string .= " /><span class=\"ec_cart_standard_shipping_price_label\">" . $express_text . " (+" . $GLOBALS['currency']->get_symbol( ) . "</span><span id=\"ec_cart_express_shipping_price\">" . $GLOBALS['currency']->get_number_only( $this->express_price ) . "</span>)</div>";
		}
		return $ret_string;
	}
	...
	public function get_shipping_price( ){
		...
		$rate = "ERROR";
		if( ( $this->shipping_method == "price" || $this->shipping_method == "weight" || $this->shipping_method == "quantity" || $this->shipping_method == "percentage" ) && $GLOBALS['ec_cart_data']->cart_data->shipping_method == "free" ){
				$rate = 0;
